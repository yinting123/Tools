...
def hasMajia(res):
    for detail in res.hotels_details:
        ...
        for room in detail.room_types:
            for product in room.products:
                if product.derivative_type == 1 or \
                    product.derivative_type == 3 or \
                    product.derivative_type == 5 or \
                    product.derivative_type == 7:
                    print 'mhotel_id:', detail.mhotel_id
...
def hasAddition(detail,id):
    for room in detail.room_types:
        for product in room.products:
            rp = product.rateplan
            for addition in rp.additions:
                if id == addition.addition_id:
                    print 'mhotel_id: ', detail.mhotel_id
                    print '     shotel_id: ', product.shotel_id
                    print '         sroom_id: ', product.sroomtype_id
...
def specialProduct(res,type):
    print 'come to this function'
    for detail in res.hotels_details:
        for room in detail.room_types:
            for product in room.products:
                # print detail.mhotel_id,'  ',product.shotel_id,'  ',product.sroomtype_id,'  ',product.rateplan.rateplan_id
                if isinstance(type,list):
                    if product.derivative_type in type:
                        print 'mhotel_id :',detail.mhotel_id
                        return True
                elif isinstance(type,int):
                    if product.derivative_type == type:
                        print 'mhotel_id :', detail.mhotel_id
                        return True
    return False
